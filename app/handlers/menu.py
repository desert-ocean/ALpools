...
BTN_WALRUS_TRAINING = "🦭 Дрессировка моржей"
...
MORGI_VIDEO_PATH = "app/data/photos/morgi.mp4"
...
def get_main_menu_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text=BTN_WALRUS_TRAINING)],
        ],
...
@router.message(F.text == BTN_WALRUS_TRAINING)
async def walrus_training(message: Message) -> None:
    if os.path.exists(MORGI_VIDEO_PATH):
        await message.answer_video(FSInputFile(MORGI_VIDEO_PATH))
    else:
